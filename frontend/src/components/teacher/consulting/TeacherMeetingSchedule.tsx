...
export default function TeacherMeetingSchedule({date, time, name, profileImgPath, isActivate}:TeacherMeetingScheduleProps){
    return <>
        <div className={`w-[300px] h-[210px] lg:m-[20px] m-[10px] ${isActivate? 'bg-gradient-to-br from-[#FFF9D7] to-[#D5E4B4]' : 'bg-[#f4f4f4]'} rounded-[20px] flex flex-col items-center justify-around p-5`}>
            <p className="text-[18px]">{date} {time}</p>
            <div className="flex flex-row items-center">
                <div className="w-[50px] h-[50px] mx-5">
                    <img src={profileImgPath} className="w-full h-full rounded-full object-cover"  />
                </div>
                <p className="font-bold text-[23px]">{name} 학부모님</p>
            </div>
            <div>
