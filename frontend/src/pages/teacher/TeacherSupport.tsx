...
export default function TeacherSupport() {
  ...
  const getStepComponent = () => {
    switch (step) {
      ...
      case 6:
        return (
          <div className="text-center mt-44 z-50"> {/* z-index를 50으로 설정 */}
  ...
  const getStepMessage = () => {
    switch (step) {
      case 2:
        return (
          <div className='z-50 text-[#2D2D2D] font-semibold'>
            <div className='text[#4A4A4A] text-[20px] mb-6'>⏰ 1단계: 상담가능시간 Open</div>
      ...
      case 3:
        return (
          <div className='z-50 text-[#2D2D2D] font-semibold'> 
            <div className='text[#4A4A4A] text-[20px] mb-6'>📅 2단계: 희망시간을 바탕으로 일정을 조율해주세요!</div>
      ...
      case 4:
        return (
          <div className='z-50 text-[#2D2D2D] font-semibold'> 
            <div className='text[#4A4A4A] text-[20px] mb-6'>💻 3단계: 상담시간에 맞춰 화상상담을 진행해보세요!</div>
            <div className='text-[16px] mb-2'><p className="text-red-500 mb-2">&lt;주의&gt;</p>폭언을 감지하여 전후를 녹화합니다. 녹화중지 버튼을 누르면 녹화를 중지합니다.</div>
      ...
      case 5:
        return (
          <div className='z-50 text-[#2D2D2D] font-semibold'>
            <div className='text[#4A4A4A] text-[20px] mb-6'>📹 4단계: 녹화된 파일을 볼 수 있어요!</div>
  ...
  return (
    ...
    >
      ...
      <div className={`relative flex flex-col items-center justify-center w-full h-full px-4 lg:px-8 ${step === 1 || step === -1 ? 'overflow-hidden' : ''}`}>
        {step === 1 && (
          <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center z-50"> {/* z-index를 50으로 설정 */}
        ...
        {helpNeeded && step > 1 && (
          <>
            ...
            {step < 6 && (
              <div className="fixed top-72 w-[520px] left-1/2 transform -translate-x-1/2 p-4 z-50">
                <div className="rounded-3xl bg-[#D6E8C9] text-white bg-opacity-85 backdrop-blur-lg border border-white border-opacity-20 p-6 shadow-lg">
                  {getStepMessage()}
                  <div className="flex justify-between mt-8">
                    <button
                      className="px-4 py-2 rounded-full bg-[#6BA96B] text-white hover:bg-[#8CAD1E] hover:text-white transition duration-300"
        ...
        <div className={`fixed ${step === 1 || step === 6 ? 'bottom-36' : 'top-60 right-60'} z-50`}>
