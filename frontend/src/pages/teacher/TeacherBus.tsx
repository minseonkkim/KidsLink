...
export default function TeacherBus() {
  ...
  useEffect(() => {
    const fetchBusStops = async () => {
      try {
        ...
        setBusStops(
          stops.map((stop) => {
            const existingStop = busStops.find(
              (prevStop) => prevStop.busStopId === stop.busStopId
            );
            return {
              ...stop,
              children: stop.children.map((child) => {
                const existingChild = existingStop?.children.find(
                  (prevChild) => prevChild.child.childId === child.child.childId
                );
                return {
                  ...child,
  ...
  const handlePrevStop = () => {
    ...
    const currentIndex = busStops.findIndex(
      (stop) => stop.busStopId === currentStopId
    );
    if (currentIndex > 0) {
    ...
    <div className="w-full max-w-md py-3 px-3 bg-white">
      ...
      {action === "출발" ? (
        <p className="text-gray-700 mb-6">
          학부모에게 버스 {action} 알림을 전송하시겠습니까?
        </p>
      ) : (
        <p className="text-gray-700 mb-6">
          아이들의 등하원 체크가 모두 완료되었습니까?
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full px-4 mt-10 lg:px-12">
        ...
        >
          <div
            className={`hidden lg:flex w-[210px] h-[330px] ${
              currentStopId !== busStops[0].busStopId
                ? "bg-[#F4F8ED]"
                : "bg-transparent"
            } rounded-[20px] items-center justify-center font-bold text-[22px]`}
          >
            {currentStopId !== busStops[0].busStopId
              ? busStops[
                  busStops.findIndex(
          ...
          <MdNavigateBefore
            className={`hidden lg:block ${
              currentStopId === busStops[0].busStopId && "invisible"
            } text-[50px] text-[#8CAD1E] cursor-pointer`}
            onClick={handlePrevStop}
          />
          <div className="bg-[#D5E4B4] rounded-[20px] w-[360px] lg:w-[420px] h-[500px] p-[20px] m-4">
            <p className="font-bold text-[24px] text-center whitespace-nowrap mb-2">
              {currentStop.busStopName}
            </p>
            <div className="bg-[#fff] rounded-[10px] w-[320px] lg:w-[380px] h-[420px] m-1 p-1 lg:p-3">
              <div className="flex flex-row my-1">
                <div className="flex items-center justify-center font-bold lg:w-[280px] w-[240px]">
                  탑승자
                </div>
                <div className="flex items-center justify-center font-bold lg:w-[60px] w-[60px]">
                  탑승여부
                </div>
              </div>
              <div className="relative w-[310px] lg:w-[360px] h-[370px] overflow-auto custom-scrollbar">
                {currentStop.children.length > 0 ? (
                  currentStop.children.map(
                    ({ child, parentTel, status, checked }, idx) => (
                      <BusChild
                        key={idx}
          ...
          <MdNavigateNext
            className={`hidden lg:block ${
              currentStopId === busStops[busStops.length - 1].busStopId &&
              "invisible"
            } text-[50px] text-[#8CAD1E] cursor-pointer`}
