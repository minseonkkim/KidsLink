...
export default function TeacherBroadcast() {
  const [user, setUser] = useState<User>({ sessionId: undefined, username: "user1" });
  const [openvidu, setOpenvidu] = useState<OpenViduState>({ session: undefined, mainStreamManager: undefined, publisher: undefined, subscribers: [] });
  const [tabOpen, setTabOpen] = useState<TabState>({ formTab: false, profileTab: false, chatTab: false });
  const [control, setControl] = useState<ControlState>({ video: false, mic: false, muted: false, volume: 0.2 });
  ...
  const joinSession = async () => {
    ...
    session
      ...
      .then(async () => {
        ...
        setOpenvidu((p) => ({ ...p, session: session, mainStreamManager: publisher, publisher: publisher }));
